...
`zero-jax` exists to address this by providing a **strictly zero external dependency** implementation of the JAX API surface. It relies solely on the Python Standard Library and `numpy` (for eager evaluations). 

Instead of wrapping heavy C++ binaries or relying on XLA, `zero-jax` mimics the public JAX API—including `jnp`, `lax`, `jit`, `grad`, and `vmap`—and acts as a pure Python frontend. 
